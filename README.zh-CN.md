...
两个解析引擎都在猜一页它没法完全理解的内容。真正重要的论文——公式密集，或者 PDF 文本层本身就是错的——还有一条更慢、但没有天花板的路：把文章读完，手写块序列和译文，编译成阅读器可以导入的文档。

```bash
python3 tools/survey.py paper.pdf                 # 先看哪几页真有插图
#   ... 读 PDF，写 script.txt ...
python3 tools/build.py paper.pdf script.txt doc.json
```

然后在首页点**导入已翻译文档**。其余功能照旧：左右对照、划词标注、点公式问 AI、深色模式。

稿子是纯文本——`@p` 是段落，`@math` 是行间公式，`@fig x0,y0,x1,y1` 按坐标裁图，`--` 分隔原文与译文。格式与写法见 [`docs/manual-translation.md`](docs/manual-translation.md)。它是为"让一个能读懂渲染后页面的助手来写"而设计的，而那恰恰是公式能抄对的前提。

### 两套解析引擎

本地引擎没有"看"的能力。它从 `getTextContent()` 拿到的只是字符和坐标，只能靠几何倒推版面 —— 字号在不在正文区间、左边界够不够顶格、行距超没超阈值。每条规则最后都落在一个数值比较上，换一份比例稍有不同的文档就整页掉进"截成图"。用几何猜语义有天花板，这个项目撞上了。
...
|---|---|---|
| **local** | 几何规则 | 墨迹投影 |
| **docvl** | 模型 —— 正文、标题、表格、阅读顺序（含双栏）、公式 LaTeX | 墨迹投影 |

**插图的位置永远不交给模型**。实测它会把一张折线图当作不存在，也会对着多面板图逐个 OCR 坐标轴标注，一路撞到 token 上限然后开始复读。所以图仍由墨迹找、按原坐标裁。

PDF.js 在模型引擎里仍然有用，只是换了用途：它的**位置**是准的，不准的是**顺序**。于是位置取自 PDF.js、顺序取自模型，两边按文字内容对上 —— 双栏页因此天然就对。

三份文档实测，可翻译字符：

| | local | docvl |
|---|---|---|
| LaTeX 论文（85 页） | 130,113 | 180,718 |
| NVIDIA 白皮书（49 页） | 65,354 | — |
| arXiv 预印本（48 页） | 59,039 | 73,273 |
| 计量论文，文本层损坏（38 页） | 52,434 | 81,119 |


第四份文档最能说明模型引擎的价值。它的 PDF 把 `∂` 存成 `@`、`≠` 存成 `6=`，希腊字母直接丢失，于是一段公式密集的正文在文本层里长这样：

```
where the total window size,=T m+ 1;is split into a pre-break window size 1=T1 m+1
```

拿去翻译只能得到噪声。模型读的是渲染后的页面：
...
代价是解析从几秒变成 48 页约 40 秒（六路并发），而且每一页的图都会发往你配置的端点。本地引擎免费、瞬时、离线，模型引擎三样都不是，但读起来好得多。

### 可选：把难啃的裁图交给文档解析模型

`getTextContent()` 只给字符和坐标。在双栏区域或表格里，它把两栏的字左右交错地吐出来 —— `control how graphics are rendered andHere's a brief overview of` —— 拿这种东西去翻译只能得到噪声。文档解析模型（PaddleOCR-VL 之类）直接读那张裁图，还回来的是干净的文字、表格行或 LaTeX。

在设置里把**文档解析模型**指向任意一个提供它的 OpenAI 兼容端点，自检就会用上。留空则什么都不变，解析全程在本地。**启用意味着这些裁图会离开本机** —— 这恰恰是这个项目平时绝不做的事，所以默认关闭，设置里也照直写明。

它被允许做的事刻意划得很窄。三份测试文档实测：

| | LaTeX 论文 | NVIDIA | arXiv |
|---|---|---|---|
| 送出去的裁图 | 33 | 12 | 29 |
| 替换掉截图 | 1 | 0 | 3 |
| 仅作附注 | 32 | 12 | 26 |
...
拿识别结果替换截图要同时满足三条：和 PDF 自带文本层对得上、内容是散文而非表格或公式、文字确实占满这块区域。差一条就只挂附注，图原样不动。

真正的价值出在附注上。回头看 README 开头那串烂掉的矩阵：

```
0.2880.4130.8991.057
```

模型还回来的是 `\hat{\mathbf{r}}_{O}=\left[\begin{array}{l}0.288\\ 0.413\\ 0.899\\ 1.057\end{array}\right]`。它会连同图片一起发给 AI 侧栏，于是"讲讲这个公式"是基于结构回答的，不再只靠像素。

实测划出的三条边界，都已写进代码：

- **公式结构对、局部错。** 那篇论文的 KKT 分块矩阵版式完全正确，但一个下标错了、一个脚注号被吸进了公式里。所以公式永远不许替换它自己的图片。
- **插图要么消失要么炸。** 一张折线图它当作不存在；一张多面板图让它一路 OCR 每个坐标轴标注，冲到 token 上限后开始复读。所以限死输出上限，并检测复读后丢弃。
- **表格 OCR 会犯普通的认错** —— `Samsung 8nm 8N` 认成了 `Samsung 8 Hm 8N`。当给 AI 的提示够用，当正文不行。
...
在三份排版体系毫无共同点的文档上验证过，同一套设置、不调任何参数：

| | 85 页 LaTeX 论文 | 49 页 NVIDIA 白皮书 | 59 页 arXiv 预印本 |
|---|---|---|---|
| 解析 | 6.1 秒 | 5.8 秒 | 8.1 秒 |
| 图形块 | 71 | 48 | 68 |
| 待译字符 | 130,113 | 65,354 | 59,039 |
| 自检可疑块 | 4 | 10 | 8 |
...
| 文件 | 职责 |
|---|---|
| `js/parser.js` | PDF → 版面块 + 裁图，纯靠几何（本地引擎） |
| `js/parser-vl.js` | 同样的输出，但由模型读版面，程序只负责裁 |
| `js/ink.js` | 墨迹投影：文本层说不清时，靠像素定位 |
| `tools/survey.py` | 逐页统计文本块、矢量绘图、位图，判断哪几页真有插图 |
| `tools/build.py` | 人工稿 + PDF → 可导入的双语文档 |
...
| `js/docvl.js` | 可选的文档解析模型客户端，以及约束它的那几道闸 |
...
**每一个宽度和缩进阈值都是地雷，索性别再依赖它们。** 第三份文档——一篇 arXiv 预印本——把整个摘要变成了一张 571pt 高的图片，连带半个引言。原因：摘要宽 325pt，而 `abstractMinW` 是 350；编号列表缩进到 x=89，而学出来的顶格线是 81。两处都只差几个点。修法是在 `classify()` 末尾加一条完全不看页面几何的兜底规则，只问三个与版面无关的问题：分了几行、这几行左边界齐不齐、文字读着像不像一段话（词够多、虚词够多、有句末标点）。表格行用自检里那套列缝判据排除。新文档待译字符因此多出 14%，另两份老文档的变化不到 1%。

**以粗体词开头的段落不是标题。** 分块逻辑原来只要"主导字体的粗细变了"就切开 —— 对真正的粗体小标题是对的，对学术写作里那种粗体引导词开头的段落（`**What we add.** This paper provides…`）就全错。那些首行被单独切出来，又因为一行、不顶格，最后变成图片。数粗体字数分不开两者：引导行的粗体字数能占到 86%。真正能分开的是粗体**结束在哪**：小标题整行收尾都粗，引导词后面必然跟着正体。改成"只在整行以粗体收尾时才切"，三份文档的标题数量一个不变（64 / 45 / 39），而那些段落全都接回去了。
